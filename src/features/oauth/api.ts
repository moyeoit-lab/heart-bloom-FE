...
export const isKakaoProvider = (
  provider: string,
): provider is typeof OAUTH_PROVIDER => {
...
export const oauthApi = {
  authorize: async ({
    ...
    redirectUri,
    state,
  }: OAuthAuthorizeRequest): Promise<void> => {
    const queryParams = new URLSearchParams({
      redirectUri,
      state,
    });
    const path = `/api/v1/auth/oauth2/${OAUTH_PROVIDER}/login-url?${queryParams.toString()}`;
    const payload = await apiRequest<OAuthAuthorizeResponse>(baseUrl, path, {
      method: "GET",
    });
  ...
  login: async ({
    ...
    code,
    redirectUri,
  }: OAuthLoginRequest): Promise<string> => {
    const queryParams = new URLSearchParams({
      code,
      redirectUri,
    });
    const path = `/api/v1/auth/oauth2/${OAUTH_PROVIDER}/login?${queryParams.toString()}`;
    const payload = await apiRequest<OAuthLoginResponse>(baseUrl, path, {
      method: "POST",
    });
